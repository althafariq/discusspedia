...
func Seed(db *sql.DB) {
	...
	// User Siswa
	rowUserSiswa, err := db.Exec("INSERT INTO users (name, email, password, role) VALUES ('admin', 'admin@gmail.com', ?, 'siswa')", hashedPassword)
	if err != nil {
		panic(err)
	}
	...
	db.Exec("INSERT INTO user_details (user_id, institute) VALUES (?, 'MIN 1 Surabaya')", userSiswaId)
